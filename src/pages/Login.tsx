    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground text-sm mt-1">{t("auth.signInDesc")}</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-4">
          ...
          <Button variant="hero" className="w-full h-12 rounded-xl mt-2" disabled={loading}>
            {loading ? <Loader2 className="size-5 animate-spin" /> : t("auth.signInBtn")}
          </Button>
        </form>

        <p className="text-center text-sm text-muted-foreground mt-6">
          {t("auth.noAccount")}{" "}
          <Link to="/signup" className="font-semibold text-foreground hover:underline">{t("auth.signUpLink")}</Link>
        </p>
      </div>
    </div>
  );
