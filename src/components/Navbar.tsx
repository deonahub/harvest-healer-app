          ...
          {!loading && (
            user ? (
              <div className="flex items-center gap-2">
                ...
                <button onClick={signOut} className="size-9 rounded-lg bg-muted flex items-center justify-center hover:bg-muted/80 transition-colors" title="Sign out">
                  <LogOut className="size-4" />
                </button>
              </div>
            ) : (
              <Link to="/login">
                <Button size="sm" className="rounded-lg h-9">Sign In</Button>
              </Link>
            )
          )}
        </nav>
